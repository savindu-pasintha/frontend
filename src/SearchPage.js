  ...
  return (
    <div className="searchPage">
      <div className="searchPage_info">
        <p>
          {numberOfDays} stays- {startdate.getDate()}th{" "}
          {months[startdate.getMonth()]} to {enddate.getDate()}th{" "}
          {months[enddate.getMonth()]} - {persons} guests
        </p>
        <h1>Stays nearby </h1>
        <div className="categories">
          {filter_category.map((fil) => (
            <span className="chip">{fil}</span>
          ))}
        </div>
      </div>
            ...
            {types.map((item) => (
              <div>
                <input
                  ...
                  onChange={add_filter}
                ></input>
                <label for={item.type_name} className="label">
                  {item.type_name}
                </label>
              </div>
            ))}
